...
### 🎙️ Multi-Speaker Podcast Generation

IndexTTS2 includes tools for generating multi-speaker podcasts from a JSON script, with optional AI-powered interjections. Scripts are located in `tools/podcast/`.

#### Script Comparison

| Script | Interjections | Trigger Detection | Calibration | Audio Smoothing |
|--------|---------------|-------------------|-------------|-----------------|
| `basic.py` | ❌ None | N/A | N/A | Basic merge |
| `llm_interjections.py` | ✅ LLM | Regex patterns | Pre-calibration per emotion | Sequential |
| `llm_advanced.py` | ✅ LLM | LLM JSON mode | On-demand | Cross-fade + logging |
| `llm_context_vibe.py` | ✅ LLM | Flexible "context_vibe" | On-demand | Cross-fade + dynamic delays |

#### Basic Multi-Speaker Synthesis (`basic.py`)

Simple multi-speaker podcast generation without interjections.

```bash
uv run tools/podcast/basic.py
```

#### LLM-Powered Interjections (`llm_interjections.py`)

Adds LLM-generated interjections with emotion-aware speech rate calibration.

**Prerequisites:**
- Install [Ollama](https://ollama.ai/) and run: `ollama serve`

```bash
uv run tools/podcast/llm_interjections.py
```

#### Advanced Version (`llm_advanced.py`) ⭐ Recommended

Uses LLM-based trigger detection with JSON mode for more accurate interjection placement.

**Features:**
- On-demand calibration (faster startup)
- Cross-fading between segments (smoother transitions)
- Interjection logging to JSON file

```bash
uv run tools/podcast/llm_advanced.py
```

#### Flexible Context Detection (`llm_context_vibe.py`)

Most advanced version with dynamic "context_vibe" detection (e.g., "shocking", "vulnerable", "sassy").

**Features:**
- No fixed trigger categories - detects any natural conversational moment
- Dynamic reaction delays based on emotional context
- Fast reactions for high-energy moments, slower for emotional ones
...
Create a JSON file with an array of speech segments:

```json
[
  {
    "speaker": "Speaker 0",
    "voice_sample": "speaker_0.mp3",
    "use_emo_text": true,
    "emo_text": "confident, clear",
    "emo_alpha": 0.6,
...
| Field | Description |
|-------|-------------|
| `speaker` | Display name for the speaker |
| `voice_sample` | Filename of the voice sample (must exist in selected speaker directory) |
| `use_emo_text` | Enable text-based emotion control |
| `emo_text` | Emotion description (e.g., "confident, clear", "warm, direct") |
| `emo_alpha` | Emotion intensity (0.0-1.0, recommended: 0.6) |
| `text` | The text to synthesize |

See `tools/podcast/example_input.json` for a complete example.


### 🔊 Speaker Extraction (Audio Splitter)

Extract individual speaker tracks from multi-speaker audio recordings. Located in `CARD-SpeakerAudioExtraction/`.

#### Installation

```bash
uv sync --extra speaker-extraction
```

#### Targeted Separation (CARD Methodology) ⭐ Recommended

The CARD methodology provides high-quality speaker separation:
...
3. **Runs SepFormer only on overlapping regions** (memory efficient)
4. **Assigns separated sources to speakers** using cosine similarity
5. **Concatenates segments with cross-fade** to avoid clicks
...
```python
from CARD-SpeakerAudioExtraction.src.separation import TargetedSpeakerSeparator
...
|--------|-------------|---------|
| `--mode` | `targeted`, `diarization-guided`, `blind`, or `auto` | `auto` |
| `--crossfade-ms` | Cross-fade duration (ms) | `25.0` |
| `--similarity-threshold` | Speaker assignment threshold | `0.7` |
| `--handle-overlap` | `skip`, `mix`, or `both` | `skip` |
| `--model` | For blind mode: `sepformer`, `dprnn-tasnet` | `sepformer` |

See `CARD-SpeakerAudioExtraction/README.md` for full documentation.
