...
def get_sentences_speaker_mapping(word_speaker_mapping, spk_ts):
    ...
    snts = []
    # CHANGE 1: Format speaker ID to match Rei's expectation (SPEAKER_00)
    snt = {"speaker": f"SPEAKER_{int(spk):02d}", "start_time": s, "end_time": e, "text": ""}
...
def cleanup(path: str):
    ...
    # [FIX] Check if it exists first. If not, just return (do nothing).
    ...
    if os.path.isfile(path) or os.path.islink(path):
        ...
        # [FIX] Just print a warning instead of crashing the whole pipeline
        print(f"[WARNING] Could not clean up {path} - unknown type")
