...
<codebaseGuide workspace="llm_test" package="card-framework" language="Python" pythonVersion="3.12+" packageManager="uv" linter="ruff">
  ...
  <section id="providers-config-and-commands" title="Providers, Config, And Commands">
    ...
    <commandNotes>
      ...
      <note>Public PyPI publishing is now driven by `.github/workflows/publish-pypi.yml`. The workflow triggers on tags matching `v*`, builds with `uv build --no-sources`, smoke-checks both the wheel and sdist for the public `infer` export, verifies that built package metadata keeps the Git-pinned `ctc-forced-aligner` requirement instead of a bare PyPI dependency, and publishes with `uv publish` using GitHub OIDC trusted publishing in the `pypi` environment.</note>
      <note>`ctc-forced-aligner` is a release-critical direct Git dependency in `pyproject.toml`, pinned to `MahmoudAshraf97/ctc-forced-aligner` commit `e23e1525bae810f0582b6e539ce7aec63fd01196`. Do not collapse it back to a bare package name unless downstream `pip install` has been revalidated, because the unrelated Deskpai `ctc-forced-aligner 1.0.2` package on PyPI fails on Windows and does not expose the API that `audio_pipeline.gateways.ctc_forced_aligner_gateway` imports.</note>
