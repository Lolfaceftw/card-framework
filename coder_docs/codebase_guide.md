...
<codebaseGuide workspace="llm_test" package="card-framework" language="Python" pythonVersion="3.12+" packageManager="uv" linter="ruff">
  <overview>
    ...
    <sessionStartChecklist>
      ...
      <step>Use coder_docs/ruff.md for lint workflow, coder_docs/uv_package_manager.md for dependency and environment workflow, coder_docs/git_github_workflow.md for contributor Git and GitHub workflow, coder_docs/github_actions_release_spec.md for GitHub Actions release/build verification, and coder_docs/scrapling.md for external web retrieval workflow.</step>
  ...
  <section id="runtime-pipeline" title="Runtime Pipeline">
    ...
    <retrievalPolicy>
      ...
      <rule>When embedding is disabled or configured as the no-op provider, the critic still receives the full transcript text directly, while the summarizer falls back to a bounded coverage-sampled transcript excerpt that preserves start, middle, and end coverage instead of retrieval results.</rule>
  ...
  <section id="audio-pipeline" title="Audio Pipeline">
    ...
    <artifacts>
      ...
      <artifact>Packaged `infer(...)` now drives the runtime through `card_framework.cli.setup_and_run`, resolving relative operator-facing inputs against the caller workspace while still pinning final packaged artifacts under the explicit infer output root.</artifact>
  ...
  <section id="providers-config-and-commands" title="Providers, Config, And Commands">
    ...
    <canonicalCommands>
      ...
      <command>git checkout -b release/vX.Y.Z</command>
    ...
    <commandNotes>
      ...
      <note>Packaged `infer(...)` now delegates to `card_framework.cli.setup_and_run` instead of invoking `card_framework.cli.main` directly, so pip-installed runs regain live wrapper/pipeline streaming while the wrapper resolves relative reusable transcripts, audio, and summary inputs from the caller workspace rather than site-packages-shaped `REPO_ROOT` paths.</note>
      ...
      <note>Packaged `infer(...)` now resolves `ffmpeg` from `CARD_FRAMEWORK_FFMPEG_EXECUTABLE`, PATH, or the bundled `imageio-ffmpeg` fallback, and it resolves `uv` from `CARD_FRAMEWORK_UV_EXECUTABLE`, PATH, or the active interpreter scripts directory before bootstrapping the vendored IndexTTS runtime.</note>
      <note>If a packaged caller selects `device=\"cuda\"`, `infer(...)` now inspects the installed PyTorch build first. When the host reports CUDA 12.6, it automatically replaces CPU-only or mismatched `torch` and `torchaudio` wheels with the CUDA 12.6 build, preferring `uv pip` inside uv-managed projects and falling back to `python -m pip` otherwise. If host detection cannot confirm CUDA 12.6, the packaged runtime still rejects unsupported builds up front.</note>
      ...
      <note>Packaged `infer(...)` no longer publishes `ctc-forced-aligner` in wheel metadata. It first tries to bootstrap the pinned upstream source on demand, preferring `uv pip --python &lt;active-interpreter&gt;` when `uv` is available, then falls back to approximate forced-alignment timing when that bootstrap cannot complete so packaged runs stay usable instead of hard-failing.</note>
      ...
      <note>Every publish attempt must start from a dedicated semver-named release branch such as `release/v1.0.5`. Prepare the version bump and release preflight on that branch, merge it, and only then create and push the matching `v1.0.5` tag.</note>
      <note>Release work is not complete at tag push time. After pushing `vX.Y.Z`, operators should watch the matching `Publish PyPI Package` run to completion with `gh run watch --exit-status` and treat failures as requiring a fix-forward new version/tag according to `coder_docs/github_actions_release_spec.md`.</note>
      <note>The public PyPI project already exists as of March 9, 2026. `1.0.1` is the first published version, `v1.0.2` failed PyPI publication, `1.0.4` is the current public release, and the next packaging fix must advance to a new version such as `1.0.5` instead of reusing a prior version number.</note>
