...
<codebaseGuide workspace="llm_test" package="card-framework" language="Python" pythonVersion="3.12+" packageManager="uv" linter="ruff">
  ...
  <section id="providers-config-and-commands" title="Providers, Config, And Commands">
    ...
    <commandNotes>
      ...
      <note>Public PyPI publishing is now driven by `.github/workflows/publish-pypi.yml`. The workflow triggers on tags matching `v*`, builds with `uv build --no-sources`, smoke-checks both the wheel and sdist for the public `infer` export, verifies that built package metadata does not leak forbidden direct URL dependencies or the incompatible bare `ctc-forced-aligner` PyPI name, and publishes with `uv publish` using GitHub OIDC trusted publishing in the `pypi` environment.</note>
      <note>The repo still uses `MahmoudAshraf97/ctc-forced-aligner` commit `e23e1525bae810f0582b6e539ce7aec63fd01196`, but that dependency now lives only in the repo-local `dependency-groups.dev` environment. Published `card-framework` wheels omit it from `Requires-Dist`, and packaged `infer(...)` bootstraps the pinned aligner on first use when stage-1 forced alignment or stage-4 interjection needs it.</note>
