...
<codebaseGuide workspace="llm_test" package="card-framework" language="Python" pythonVersion="3.12+" packageManager="uv" linter="ruff">
  ...
  <section id="providers-config-and-commands" title="Providers, Config, And Commands">
    ...
    <commandNotes>
      ...
      <note>Packaged `infer(...)` no longer publishes `ctc-forced-aligner` in wheel metadata. It first tries to bootstrap the pinned upstream source on demand, then falls back to approximate forced-alignment timing when that bootstrap cannot complete so packaged runs stay usable instead of hard-failing.</note>
