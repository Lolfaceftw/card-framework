...
This helper shells out to ``setup_and_run.py`` for each ordered model pair so it
reuses the repository's normal bootstrap, stage selection, merged live-draft
stage-2/stage-3 flow, and summarizer/critic orchestration path. The helper
disables only stage-4 interjector output, then copies the resulting
``summary.xml`` into a pair-specific artifact file whose name starts with
...
class ModelProfile:
    ...
    def build_provider_overrides(self, *, base_key: str) -> list[str]:
        ...
        if self.provider_target == "providers.vllm_provider.VLLMProvider":
...
DEFAULT_VLLM_MODELS: tuple[ModelProfile, ...] = (
    ModelProfile(
        ...
        provider_target="providers.vllm_provider.VLLMProvider",
        port=8000,
    ),
    ModelProfile(
        slug="qwen3_5_9b",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Run setup_and_run.py across the built-in summarizer/critic model "
            "matrix and save one summary XML per ordered pair."
        )
    )
    parser.add_argument(
        "--vllm-host",
    ...
    parser.add_argument(
        ...
        metavar="KEY=VALUE",
        help=(
            "Extra Hydra override forwarded to every setup_and_run.py invocation. "
    ...
    parser.add_argument(
        "--uv-executable",
        default="uv",
        help="uv executable to use for child setup_and_run.py calls.",
    )
    parser.add_argument(
        "--fail-fast",
        action="store_true",
...
def build_setup_command(
    *,
    ...
    """Build one setup_and_run.py subprocess command for a model pair."""
    ...
    command = [args.uv_executable, "run", "python", "setup_and_run.py"]
...
def build_child_env(args: argparse.Namespace, *, deepseek_api_key: str) -> dict[str, str]:
    """Build the environment used by child setup_and_run.py invocations."""
    env = dict(os.environ)
    ...
    *,
    ...
    if not SUMMARY_SOURCE_PATH.is_file():
        return PairResult(
            ...
            input_path=str(resolved_input.path),
            returncode=returncode,
            error="setup_and_run.py completed but summary.xml was not created.",
        )
