...
def load_audio_config(project_root: Path) -> dict[str, Any]:
    ...
    """
    config_path = (project_root / "conf" / "config.yaml").resolve()
    if not config_path.exists():
        raise FileNotFoundError(f"Config file not found: {config_path}")
    config = OmegaConf.load(config_path)
    resolved = OmegaConf.to_container(config, resolve=True)
    if not isinstance(resolved, dict):
        raise ValueError("conf/config.yaml did not resolve to a mapping.")
    audio_cfg = resolved.get("audio", {})
    if not isinstance(audio_cfg, dict):
        raise ValueError("conf/config.yaml must define an audio mapping.")
    return audio_cfg
